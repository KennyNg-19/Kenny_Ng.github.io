...
- **Pre-Processing work** 自然语言理解的**前处理**的任务，包括：

  - **Parsing 句法分析**

    给定句子，根据<u>词性</u>生成**树状结构** by **CYK算法**

  - **Dependency Parsing**(词的)**依存**分析 
...
- **Relation Extraction**(重要): 关系抽取
  ...
  关系抽取的输入：**序列和抽取出的实体**，输出是**两两实体之间的关系**。本质是一个**<u>分类</u>任务**
...
#### 原理：**<u>全排列组合</u>**分词后的句子，用语言模型从中筛选出语义最合适的



第一步通过**翻译模型TM**进行**分词**

第二步**映射**词语，因为语法的存在而进行<u>映射后词语的排列组合</u>

第三步通过**语言模型LM**选出排列组合中最合适的语句

![](https://tva1.sinaimg.cn/large/007S8ZIlgy1gf99soj3vmj317w0kuq8d.jpg)缺点: 排列组合的复杂度太高

解决: **维特比Viterbi**算法——**同时考虑TM**分词+**LM筛选**，这分开的2个步骤
...
![模型的输入：特征提取很重要](https://tva1.sinaimg.cn/large/007S8ZIlgy1gfjqjhtaemj31kj0u0qte.jpg)
