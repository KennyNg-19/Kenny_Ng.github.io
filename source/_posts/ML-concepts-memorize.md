...
估计（estimating）是一种手段，一种对**模型参数**进行**推测(inference)**的手段，说白了，就是利用训练数据X，对模型进行calibration(校正)。在这之前，首先**需要有一个<u>种类确定</u>的模型**，推测的目标是模型中**待辨识的**参数。



#### 极大似然估计公式

先看似然函数的定义，它是给定联合样本值x下关于(未知)参数θ的函数，人话说就是：**利用已知的样本结果 x**，反推最有可能（最大概率）**<u>导致</u>这样结果的参数**

![ML Estimation公式](https://tva1.sinaimg.cn/large/0081Kckwgy1gl6k5alhoej30ou05k75h.jpg)

- 这里的小**x**指联合样本随机变量X取到的值，即X=x；

- 联和样本集**X**={x1 、x2 、…、xn}，**MLE<u>绝对默认假设</u><u>样本之间都是相对独立的</u>，注意这个假设很重要！所以上面才能连乘**

- 这里的θ是指未知参数，属于参数空间；

- 这里的f(x|θ)是一个**我们理解的频率\似然值**，即P(x|θ) 或者 **直接写P(x)**，因为根据定义似然就是**在假设模型未知θ下**，得到的**data X的分布**！所以**未知θ，但却可以写P(x|θ)**； 既然data X已知，可通过**count频率直接得到，某个x的**似然值
...
**贝叶斯派机器学习**：**MAP，极大后验估计**，**对概率公式积分——核心求的是<u>积分</u>**
...
条概y|x 总是给人一种**"先有x后有y"**的感觉，但其实x,y**并不是先有鸡后有蛋的关系**——x,y应该理解**<font color="red">为y受到x的约束</font>，会随着x的改变而y的取值也改变**。条件所表达的意义**在于约束，而非顺序**
